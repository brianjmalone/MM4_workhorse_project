...
# Builds ARM64 image (native for M4) for better performance - NO Rosetta translation needed
...
if [ -z "$NOTEBOOK" ] || [ -z "$OUTPUT_NAME" ]; then
    echo "Usage: ./scripts/run-remote-buildx.sh NOTEBOOK OUTPUT_NAME [--timestamp]"
    echo ""
    echo "Examples:"
    echo "  ./scripts/run-remote-buildx.sh notebooks/23_train_model.ipynb xgboost_baseline"
    echo "  ./scripts/run-remote-buildx.sh notebooks/23_train_model.ipynb xgboost_lr001 --timestamp"
    echo ""
    exit 1
fi
...
echo "=== Running notebook on Mac Mini (ARM64 native) ==="
echo "Notebook: $NOTEBOOK"
echo "Results: results/$RUN_ID/"

# Build ARM64 Docker image on iMac using buildx
# - `docker buildx build`: Multi-platform build tool (cross-compilation support)
# - `--platform linux/arm64`: Build for ARM64 architecture (M4's native arch)
# - `-t analysis:latest`: Tag the image as "analysis:latest"
# - `--load`: Load built image into local Docker (needed to transfer it via `docker save`)
# - `.`: Build context (current directory)
# - On Intel Mac: Uses QEMU emulation to build ARM images (slower build, faster runtime on M4)
# - First build is slow, but subsequent builds use layer caching
echo "Building ARM64 Docker image..."
docker buildx build --platform linux/arm64 -t analysis:latest --load .
...
# - ARM64 image runs NATIVELY on M4 (no Rosetta translation = better performance)
